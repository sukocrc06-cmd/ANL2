...
const translations = {
  ...
  badge_sandbox: {
    tr: "İnteraktif Laboratuvar",
    en: "Interactive Sandbox"
  },
  sandbox_title: {
    tr: "İnteraktif Analitik Laboratuvarı",
    en: "Interactive Analytics Sandbox"
  },
  sandbox_desc: {
    tr: "Sisteme giriş yapmadan önce yapay zeka modellerimizin çalışma prensibini canlı simülasyonla test edin.",
    en: "Simulate a live machine learning model. Adjust inputs below to see predictions update in real-time before accessing the portal."
  },
  tab_egitim_short: {
    tr: "Eğitim (Olasılık)",
    en: "Education (Prob)"
  ...
  tab_vakif_short: {
    tr: "Vakıf (Sınıflandırma)",
    en: "Charity (Classify)"
  },
  sandbox_prediction_title: {
    tr: "Simülasyon Çıktısı",
    en: "Simulation Prediction"
  },
  btn_sandbox_guide_start: {
    tr: "Aura AI Sesli Rehberi Başlat",
    en: "Start Aura AI Voice Guide"
  },
  sandbox_guide_placeholder: {
    tr: "Yapay zeka sesli/görsel rehberini başlatmak için yukarıdaki butona tıklayın. Modelin karar sürecini detaylıca açıklayacaktır.",
    en: "Click the button above to start the AI voice/visual guide. It will explain the model decision-making process in detail."
  },
  btn_viz_default: {
    tr: "Sektör Modeli",
    en: "Sector Model"
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (sectorSelect) {
    sectorSelect.addEventListener('change', (e) => {
      ...
      // Sync with Welcome Page Onboarding Sandbox tab selection
      if (typeof WelcomeSandbox !== 'undefined' && WelcomeSandbox.setActiveTab) {
        WelcomeSandbox.setActiveTab(selectedSector);
      }
  ...
  function initializeNavigation() {
    ...
    document.querySelectorAll('.menu-item').forEach(item => {
        item.addEventListener('click', function(e) {
            ...
            // Sync route hash so panel is properly mounted/initialized
            let panelKey = '';
  ...
  // Play procedural synthesized blip sounds on typewriter keys using Web Audio API
  ...
  function applyHighlight(selector) {
    // Clear all simulation highlights first
    ...
    if (selector) {
      document.querySelectorAll(selector).forEach(el => {
        el.style.setProperty('box-shadow', '0 0 25px #0ea5e9', 'important');
        el.style.setProperty('border', '2px solid #0ea5e9', 'important');
        el.style.setProperty('transition', 'all 0.5s ease', 'important');
  ...
  function startLiveSimulationEngine() {
    ...
    if (dashOutputResult) {
      ...
      if (parentCard) {
        parentCard.style.setProperty('background', 'rgba(16, 185, 129, 0.15)', 'important');
        parentCard.style.setProperty('border', '1px solid #10b981', 'important');
        parentCard.style.setProperty('box-shadow', '0 0 20px rgba(16, 185, 129, 0.25)', 'important');
      }
    }
    ...
    if (typeof updatePerformanceMetrics === 'function') {
      updatePerformanceMetrics(true, true);
    }

    // Create/Show floating tour banner
    ...
    bannerText.style.cssText = `
      ...
      color: var(--text-primary);
    `;
    bannerText.textContent = 'Aura AI Canlı Simülasyonu Aktif';
    ...
    `;
    stopBtn.textContent = 'Turu Kapat';
    stopBtn.addEventListener('click', teardownSimulationEngine);
  ...
  function teardownSimulationEngine() {
    ...
    // Reset database mock data
    if (typeof databases !== 'undefined') {
      const activeSector = (typeof currentSector !== 'undefined' && currentSector) || 'vakif';
    ...
    // Reset prediction results text & parent output card style
    const dashOutputResult = document.getElementById('dash-output-result');
    if (dashOutputResult) {
    ...
    // Smoothly bring back empty spreadsheet dropzone layer
    const emptyState = document.getElementById('dashboard-empty-state');
    if (emptyState) {
  ...
  // Companion Sandbox mock helper object to handle calls gracefully
  const WelcomeSandbox = {
    activeSector: 'vakif',
    init: function() {
      // Event delegation handles events globally, no direct binding required
    },
